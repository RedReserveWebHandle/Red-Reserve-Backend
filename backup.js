...
app.get('/api/donor/lastdonation', verifyToken, async (req, res) => {
    const donor = await Donor.findOne({ email: req.user.email })
    if (!donor || !donor.profile) {
        return res.status(404).json({ message: 'Donor not found or profile missing [404]' })
    }
